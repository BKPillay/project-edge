# EDGE AI v7.1 PATCHED EDGE MODEL
...
def add_draw_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["structure"] = (
        out["odd_count"].astype(str) + " Odd / " + out["even_count"].astype(str) + " Even · "
        + out["low_count"].astype(str) + " Low / " + out["high_count"].astype(str) + " High"
    )
...
def number_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    r20_s = minmax(r20_counts)
    r50_s = minmax(r50_counts)
    r100_s = minmax(r100_counts)
    gap_s = minmax(gaps)
    ...
    for n in NUMBERS:
        ...
        rows.append({
            ...
            "Long Count": long_counts[n],
            "Last 20": r20_counts[n],
            "Last 50": r50_counts[n],
            "Last 100": r100_counts[n],
            "Gap": gaps[n],
            "Reason": number_reason(long_counts[n], r20_counts[n], r50_counts[n], gaps[n]),
        })
...
def pair_predictions(df: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    ...
    for pair, count in pc.items():
        ...
        rows.append({
            ...
            "Score": round(score * 100, 2),
            "Appeared": count,
            "Last Seen": "latest draw" if gap == 0 else f"{gap} draws ago",
            "Reason": "Frequent pair" if count >= percentile_90 else "Pair + gap balance",
        })
...
def triplet_predictions(df: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    ...
    for triplet, count in tc.items():
        ...
        rows.append({
            ...
            "Appeared": count,
            "Last Seen": "latest draw" if gap == 0 else f"{gap} draws ago",
            "Reason": "Recurring triplet" if count >= percentile_95 else "Triplet + gap balance",
        })
...
def over_under_prediction(df: pd.DataFrame, threshold: float = 92.5) -> dict:
    ...
    return {
        ...
        "recent_20_over_rate": round(r20_rate * 100, 2),
        "recent_50_over_rate": round(r50_rate * 100, 2),
        "recent_100_over_rate": round(r100_rate * 100, 2),
        "recent_avg_sum_10": round(avg10, 2),
    }


def walk_forward_over_under_backtest(df: pd.DataFrame, min_train: int = 200) -> pd.DataFrame:
    rows = []
    for i in range(min_train, len(df)):
        train = df.iloc[:i].copy()
        pred = over_under_prediction(train)
        actual_sum = int(df.iloc[i][NUMBER_COLS].sum())
        actual = "Over 92.5" if actual_sum > 92.5 else "Under 92.5"
        rows.append({
            "draw_date": df.iloc[i]["draw_date"],
...
def simple_top3_backtest(df: pd.DataFrame, min_train: int = 200) -> pd.DataFrame:
    rows = []
    for i in range(min_train, len(df)):
        train = df.iloc[:i].copy()
        actual = set(map(int, df.iloc[i][NUMBER_COLS].tolist()))
        top3 = number_scores(train).head(3)["Number"].astype(int).tolist()
        rows.append({
            "draw_date": df.iloc[i]["draw_date"],
            "top3": "-".join(map(str, top3)),
            "matches": len(actual & set(top3)),
        })
    return pd.DataFrame(rows)


def performance_summary(df: pd.DataFrame) -> dict:
    ou = walk_forward_over_under_backtest(df)
    t3 = simple_top3_backtest(df)

    return {
        "over_under_accuracy": round(float(ou["correct"].mean()) * 100, 2) if len(ou) else None,
        "over_under_tests": int(len(ou)),
        "top3_avg_matches": round(float(t3["matches"].mean()), 3) if len(t3) else None,
        "top3_2plus_hit_rate": round(float((t3["matches"] >= 2).mean()) * 100, 2) if len(t3) else None,
        "top3_tests": int(len(t3)),
    }
